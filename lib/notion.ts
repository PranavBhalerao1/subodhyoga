...
function getFileProperty(
  ...
  try {
    const fileUrl = Array.isArray(raw)
      ? (raw?.[0] as unknown[] | undefined)?.[1] instanceof Array
        ? (
            ((raw?.[0] as unknown[] | undefined)?.[1] as unknown[])?.[0] as
              | unknown[]
              | undefined
          )?.[1]
        : undefined
    ...
    if (typeof fileUrl === "string" && fileUrl.length > 0) {
      return normalizeImageUrl(fileUrl);
    }
  } catch {
    // fall through to text extraction
  }

  const fallbackText = getTextContent(
    raw as Parameters<typeof getTextContent>[0]
  ).trim();
  if (fallbackText) {
    return normalizeImageUrl(fallbackText);
  }
